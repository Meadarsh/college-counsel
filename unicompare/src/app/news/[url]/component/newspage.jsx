...
const Newspage = async ({ params }) => {
  ...
  let latestList;
  try {
    const res = await fetch(`${process.env.BASE_URL}/api/news/${params?.url}`, {
      cache: "no-store",
    });
  ...
  const articleSchema = {
    ...
    "publisher": {
      ...
      "logo": {
        "@type": "ImageObject",
        "url": "https://unicompare.co.in/logo/cc.png"
      }
    },
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `${process.env.BASE_URL}/news/${params?.url}`
    }
  };
        ...
        <article className="w-full flex gap-4">
          <main className="lg:w-[75%] w-full">
            <Image
              ...
              alt={`${blog?.title} - UniCompare News`}
              title={blog?.title}
              loading="eager"
              quality={90}
