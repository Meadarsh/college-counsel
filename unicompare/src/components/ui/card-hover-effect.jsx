...
export const HoverEffect = ({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4  lg:p-10 xl:grid-cols-6  xl:p-10  ",
        className
      )}
    >
      {items.map((item, idx) => (
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-primary/10 dark:bg-slate-800/[0.5] block  rounded-2xl"
