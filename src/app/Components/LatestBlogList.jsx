...
const LatestBlogList = ({ List }) => {
  return (
    ...
    <div className="flex flex-col h-[calc(100%-64px)] overflow-y-scroll gap-1 p-1">
        {List?.map((data) => (
           <Link href={data.url}  style={{ textDecoration: 'none', color: 'inherit' }} ><div key={data.id}  className="flex gap-2 py-2 px-1 bg-white rounded-md">
           <Image
             width={150}
           ...
           />
           <div>
             <h5 className=" leading-snug text-[15px] font-bold">{data?.title}</h5>
             <p className="text-[10px] mt-1 text-gray-400">{data?.upload_time}</p>
           </div>
         </div></Link> 
        ))}
    </div>
